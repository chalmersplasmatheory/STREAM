		...
		ccdIon = adas->GetCCD(1,1);
            ...
            if (Z == 1){ //Deuterium or Tritium
		        ...
		        ccd = GetCCD(iz);
                    ...
                    for(len_t Z0i=1; Z0i<Zi+1; Z0i++){ //Loop over all charge states of other ion
                        ...
                        if (Z0 == 0) {
                            ...
                            NI_Z(iz, 0, -PartialTRcx * (-2.0*W_i[iz*Nr+ir]/(3.0*ec*N_i_temp*N_i_temp)) * V_n/V_n_tot * nions[(IonOffset+Z0i)*Nr+ir]); //First argument is 0 since we want the neutral density for D/T (and we have Z0=0 here)

                            // D-T term (absent in DYON)
                            if (Zi == 1)
                                NI_Z(iIon, 1, PartialTRcx_ion * (-2.0*W_i[iIon*Nr+ir]/(3.0*ec*N_i[iIon*Nr+ir]*N_i[iIon*Nr+ir])) * V_n_iz/V_n_tot * nions[(IonOffset+0)*Nr+ir]);
                        } else if (Z0 == 1) {
                            // Apply to neutral deuterium (Z0-1 = 0)
                            NI_Z(iz, -1, PartialTRcx * (-2.0*W_i[iz*Nr+ir]/(3.0*ec*N_i_temp*N_i_temp)) * V_n/V_p * nions[(IonOffset+Z0i)*Nr+ir]); //First argument in NI 0 because we want the neutral density for D/T (and we have Z0=1 here)

                            // D-T term (absent in DYON)
                            if (Zi == 1)
                                NI_Z(iIon, 0, -PartialTRcx_ion * (-2.0*W_i[iIon*Nr+ir]/(3.0*ec*N_i[iIon*Nr+ir]*N_i[iIon*Nr+ir])) * V_n/V_p * nions[(IonOffset+0)*Nr+ir]);
                        }
                    }
                }
                ...
                for (len_t iz=0; iz<NZ; iz++){ //Loop over all other ion species
                    ...
                    const real_t V_n_iz = this->volumes->GetNeutralVolume(iz); 
                    if (Z0 == 0){
                        NI_Z(iIon,+1, PartialTRcx_ion * (-2.0*W_i[iIon*Nr+ir]/(3.0*ec*N_i[iIon*Nr+ir]*N_i[iIon*Nr+ir])) * V_n_iz/V_n_tot * nions[Doffset*Nr + ir]); 
                    }else{
                        NI_Z(iIon,+1, PartialTRcx_ion * (-2.0*W_i[iIon*Nr+ir]/(3.0*ec*N_i[iIon*Nr+ir]*N_i[iIon*Nr+ir])) * V_n_iz/V_p * nions[Doffset*Nr + ir]);
                    }
                }
            }
            ...
            if (Z != 1 && Z0 >= 1){  //Not Deuterium/Tritium. Z0>1 since this term not present if Z0=0
                ...
                for (len_t iz=0; iz<NZ; iz++){ //Loop over all other ion species
                    ...
                    const len_t Doffset = ions->GetIndex(iz,0); //Get index of neutral state of D
                    const real_t V_n_iz = this->volumes->GetNeutralVolume(iz); 
                    NI_Z(iIon, 0, -PartialTRcx_ion * (-2.0*W_i[iIon*Nr+ir]/(3.0*ec*N_i[iIon*Nr+ir]*N_i[iIon*Nr+ir])) * V_n_iz/V_p * nions[Doffset*Nr + ir]); 
