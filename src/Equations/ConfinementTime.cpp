...
real_t ConfinementTime::EvaluateConfinementTime(len_t ir) { 
		/**
		* Evaluates the inverted confinement time 1/tau = 1/tau_par + 1/tau_perp
		*/
...
real_t ConfinementTime::Goldstone_scaling(const PowerList& coeff, len_t ir, 
	...
	*/
	...
	real_t Ip = Conv_mega * unknowns->GetUnknownData(id_Ip)[ir];
	real_t ne = ConvUnit * unknowns->GetUnknownData(id_Ncold)[ir];
	real_t Pm = GetOhmicPower(ir) * Conv_mega;
	real_t A = DREAM::Constants::mD / DREAM::Constants::mu;  // Atomic mass of deuterium
	real_t prod = coeff[0];
...
real_t ConfinementTime::ITER89_OL_ConfinementTime(len_t ir) const {		
	...
	*/
	real_t P = GetOhmicPower(ir) * Conv_mega;
