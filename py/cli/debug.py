...
from DREAM import setup_interactive, who
...
def collect_single(name, output):
    ...
    of STREAMOutput objects 'output'.
    """
    data = []
    for do in output:
...
def load_quantities(directory, timestep):
    ...
    do = STREAMOutput(pattern.format(timestep, 1), loadsettings=True)

    # Load all output objects
    dos = [do]
    iteration = 1
    while os.path.exists(pattern.format(timestep, iteration)):
        dos.append(STREAMOutput(pattern.format(timestep, iteration), loadsettings=False))
        iteration += 1
...
def plotConvergenceOutput(do):
    """
    Plot the convergence of the unknowns in the given STREAMOutput object.
    """
...
def plotConvergence(u=None):
    """
    Plot the convergence of a given unknown quantity of STREAMOutput object.
    """
    if u == None:
        u = globals()['do']
    
    if type(u) == STREAMOutput:
...
def create_argparser():
    parser = argparse.ArgumentParser(description="STREAM Debug Output CLI")
