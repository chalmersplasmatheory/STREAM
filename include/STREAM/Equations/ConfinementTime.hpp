...
namespace STREAM
{
	// To avoid to include the file -> circular definitions
    ...
    class ConfinementTime  {
        protected:
            ...
            // Computes the ohmic power
            real_t GetOhmicPower(len_t ir) const; 
			
            // Computes the confinement time from Bohm
            real_t Bohm_ConfinementTime(len_t ir) const; 
                        
            // Computes INTOR scaling law
            real_t INTOR_ConfinementTime(len_t ir) const; 

            real_t ITER89_OL_ConfinementTime(len_t ir) const; 
            
            // Computes Odajima-Shimomura offset linear scaling law
            real_t OS_OL_ConfinementTime(len_t ir) const; 
			
            // Computes Rebut-Lallia offset linear scaling law
            real_t RL_OL_ConfinementTime(len_t ir) const; 
            
            // Computes Rebut-Lallia-Watkins scaling law
            real_t RLW_ConfinementTime(len_t ir) const; 
