...
def generate(prefill=1e-6, gamma=2e-3, Vloop=12, Vloop_t=0, I0=2.4e3, tmax=1e-4, nt=4000, EfieldDYON=True, runaways=True):
    """
    ...
    ss.eqsys.n_i.ions[iD].setRecyclingCoefficient('D', 1.0)
...
def drawFhot(axs, so, toffset=0):
    so.eqsys.f_hot.semilogy(t=[0, 100, 250, 500], ax=axs)
    axs.set_title('Distribution function')
...
def makeplots(so1, so2, so3=None, save=False, fileaddon=''):
    '''
    ...
    drawplot4(axs4, so1, showlabel=True, save=save, fileaddon=fileaddon)
    drawplot4(axs4, so2, toffset=so1.grid.t[-1], showlabel=False, save=save, fileaddon=fileaddon, first=False)
    if so3 is not None:
        drawplot4(axs4, so3, toffset=so1.grid.t[-1]+so2.grid.t[-1], showlabel=False, save=save, fileaddon=fileaddon, first=False)
        fighot, axshot = plt.subplots(1, 1, figsize=(4, 3))
        drawFhot(axshot, so3)
...
def main(argv):
    ...
    if settings.skip is None or (len(settings.skip) > 0 and 2 not in settings.skip):
        ...
        ss1.save(f'settings1{ext}_grid.h5')
        so1 = runiface(ss1, f'output1{ext}_grid.h5', quiet=False)

        ss2 = STREAMSettings(ss1)
        ss2.fromOutput(f'output1{ext}_grid.h5')
        ss2.timestep.setTmax(0.004 - ss1.timestep.tmax)
        ss2.timestep.setNt(400)
        ss2.save(f'settings2{ext}_grid.h5')
        so2 = runiface(ss2, f'output2{ext}_grid.h5', quiet=False)
        #'''
        ...
        #'''
        ss3.save(f'settings3{ext}_grid.h5')
        so3 = runiface(ss3, f'output3{ext}_grid.h5', quiet=False)
        #'''

    else:
        so1 = STREAMOutput(f'output1{ext}.h5')
        so2 = STREAMOutput(f'output2{ext}.h5')
        so3 = STREAMOutput(f'output3{ext}.h5')

    if settings.plot:
        makeplots(so1, so2, so3)
